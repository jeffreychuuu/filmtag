...
| 功能                        | 說明                                                                                                                                                        |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 📷 相機 & 鏡頭              | 相機係成卷揀一次（設喺上傳區上面）— 內建 Leica MP、Olympus OM-2Sp 等，亦支援完全自訂相機型號、鏡頭焦距同最大光圈。鏡頭係逐張相揀：揀鏡頭 modal 只顯示嗰部機啱用嘅鏡頭，所以同卷同機都可以用唔同鏡頭                                                              |
...
- 📷 鏡頭當作逐張相設定：揀鏡頭 modal 淨係顯示所選嗰部機嘅鏡頭（之前仲可以喺 modal 入面換機）
- 🗂️ 版面重新分做兩大視覺組：**Photos & Gear**（上傳、file list、相機）同 **Metadata**（藝術家、底片、沖掃、沖洗、增感/減感、掃描器、簽名），用 icon 做 group header
- 🔓 Metadata 改用 progressive reveal — 未上傳時鎖住，上傳完自動解鎖（清空/刪相時會再鎖返）
- ⚠️ 當一卷已經用咗多過一支鏡頭，轉相機而家會彈確認視窗，而唔係靜靜雞重設所有鏡頭
