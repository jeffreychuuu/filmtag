...
function updatePhotoCount(n) {
  fetch("/api/count", {
    ...
    body: JSON.stringify({ type: "photos", amount: n }),
  })
    .then(function (r) {
      return r.json();
    })
      ...
      try {
        ...
        if (!jpegStr) {
          for (var b = 0; b < bytes.length; b++)
            jpegStr += String.fromCharCode(bytes[b]);
        }
        var exifObj;
        try {
        ...
        var dt =
          fd.exifDate +
          " " +
          String(fd.hr).padStart(2, "0") +
          ":" +
          String(fd.min).padStart(2, "0") +
          ":00+08:00";
        ...
        if (p.camera.shutter) {
          ...
          if (sf.length === 2) {
            exifObj["Exif"][piexif.ExifIFD.ExposureTime] = [
              parseInt(sf[0], 10),
              parseInt(sf[1], 10),
            ];
            exifObj["Exif"][piexif.ExifIFD.ShutterSpeedValue] = [
              parseInt(sf[0], 10),
              parseInt(sf[1], 10),
            ];
          }
    ...
    function (p, zip, total) {
      addContentSheetIfEnabled(S.uploadedFiles, p, zip, null, function () {
        ...
        zip.generateAsync({ type: "blob" }).then(function (blob) {
          ...
          var spin = S._("progress-spinner"),
            done = S._("progress-done");
...
export function startSaveProcess() {
  ...
  runProcessPipeline(
    function (bytes, nn, entry) {
      S.processedFiles.push({
        name: nn,
        blob: new Blob([bytes], { type: entry.file.type || "image/jpeg" }),
      });
    },
    function (p, zip, total) {
      addContentSheetIfEnabled(
        S.uploadedFiles,
        p,
        zip,
        S.processedFiles,
  ...
  for (var i = 0; i < files.length; i++) {
    (function (f) {
      ...
      canvas.addEventListener("click", function (e) {
        ...
        S.imgOverlayImg.onload = function () {
          URL.revokeObjectURL(url);
        };
...
export function showStatus(msg, type) {
  S.statusMsg.textContent = msg;
  S.statusMsg.className = "status-msg " + type;
  S.statusMsg.style.display = "block";
}
...
export function generateContentSheet(files, params, onComplete) {
  ...
  var mx = Math.round(cw * 0.025),
    my_t = Math.round(ch * 0.04),
    my_b = Math.round(ch * 0.1);
  var cg = Math.round(cw * 0.01),
    rg = Math.round(ch * 0.015);
  ...
  function tryFinish() {
    ...
    if (minD !== null && maxD !== null)
      dr = fmtDateStr(new Date(minD)) + " ~ " + fmtDateStr(new Date(maxD));
    ctx.fillText(
      "\uD83E\uDDEA " + params.lab + "  |  \uD83D\uDCC5 " + dr,
      lp,
      footerY + fSize * 3 + 16,
    ...
    while (active < 4 && next < n) {
      ...
      (function (i) {
        ...
        img.onload = function () {
          ...
          var scale = Math.min(cell_w / fitW, cell_h / fitH) * 0.8;
